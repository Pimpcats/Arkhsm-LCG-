...
| Official reference cards | ✅ vendored: `docs/art_reference/` (6 cards straight from the game) + `CARD_ANATOMY.md` — the target layouts and their required fields. |
| Print layer | ✅ `pipeline/stillhour_print_text.json` — rules text (errata-grade, `[wil]` markup), flavor, investigator back text, and enemy combat stats for all 36 cards; feeds the SE frame jobs AND the placeholder renderer. FLAG: The House Always Wins statline is a DRAFT (no source in the design docs). |
