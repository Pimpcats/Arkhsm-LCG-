# THE STILL HOUR — Campaign Guide
*A campaign for 1–4 investigators, best with 3.*

---

## HOW TO USE THIS GUIDE

This guide is read as you play. **Do not read ahead.** It tells you when to read each section: the Prologue first, then the loop, then the districts as you reach them. Story text in boxes is read aloud. Everything after a **"Do not read until…"** line stays unread until the guide sends you there.

The Still Hour is not a line of separate scenarios. It is **one night, lived over and over.** You will play the Prologue once, then play the night again and again (each play-through is a **loop**), learning a little more of it each time, until you know enough to end it. Every loop costs you years.

Along the way the guide will ask you to **make choices**. Record each one in your Campaign Log. Nothing you choose in Ambergrove stays where you left it: every choice returns somewhere else in the town, and all of them return at the end.

---

## CAMPAIGN RULES

**Loops.** A loop is one continuous play-through of the night across the shared Ambergrove map. The **Hourglass** (the Occultation clock, Hours I–IX) is the master timer for the whole loop. When the Hourglass reaches **Hour IX — The Appointed Hour**, when every investigator is defeated, or when Dissonance reaches **18**, the loop ends and the night **resets**. The board wipes and everyone is healed, but **Memory, the Knowledge Track, Dissonance's scar and Years all carry forward.**

**The board persists within a loop.** Damage, horror, resources and cards in play do **not** reset when you travel between districts. The reset is your only breather, and your only checkpoint.

**Districts and travel.** Ambergrove is a small inland town of six **districts**. Each is a cluster of locations with things to learn and do, set out from its own box. **The Square** is the hub. Travelling along a connection between districts costs **1 Hour** (a Skip). You will only reach 2 or 3 districts before time runs out.

**The Hourglass.** The clock advances about one Hour of its own accord each round (each Hour agenda has a doom threshold of 1), plus any **Skip** effects. When the Hourglass reaches an Hour, resolve that Hour's text.

**Dissonance.** A shared track from 0 to 18. It rises when you act on foreknowledge: playing Recollections, using foreknowledge abilities, and certain encounter cards. It only falls through specific cards. It sets the chaos bag's **band**:

| Band | Dissonance (3 investigators) | Effect |
|---|---|---|
| Calm | 0–5 | Echoes are Sleepwalking. |
| Glitch | 6–11 | 1 [static] token in the chaos bag. Echoes wake. |
| Noticed | 12–17 | 2 [static] tokens in the chaos bag. The Appointed has Arrived. |
| Reset | 18 | The loop ends at once. |

A **[static]** token is −3; when it is revealed, raise Dissonance by 1. The Control token tracks Dissonance and manages the band's [static] tokens for you.

**Memory.** Memory tokens you earn during a loop sit on your cards. Between loops they are **banked**. Banked Memory is capped at **18** (6 × investigators) at the start of each loop. It is spent like experience: on **Recollections** at their listed Memory cost, and on card upgrades at 1 Memory per level. On-card Memory (fuel during a loop) and banked Memory (the campaign's currency) are always named separately.

**The Knowledge Track.** The night can be learned. Completing a district's objectives records **Knowledge** in your Campaign Log, and each piece of Knowledge permanently changes the night from then on. Knowledge is never bought; it is only found.

**Years.** At each reset every investigator gains **Years**: +1 always, +1 if they were defeated that loop, +1 if the loop ended at Dissonance 12 or higher, and +1 if they leaned on the loop (raised Dissonance 3 or more times, or spent 4 or more Memory). Years set an investigator's age bracket: **Prime 0–4, Weathered 5–9, Elder 10–14, Ancient 15+**, each with its own drift. An investigator who reaches **18 Years** ages out of the campaign.

**Echoes and the Appointed.** Echoes are **Sleepwalking** while Dissonance is Calm and wake in the Glitch band or higher. Something called **the Appointed** climbs an **Approach** (Unseen, Sensed, Emerging, Arrived), driven up by the clock and by Dissonance. It cannot be defeated, only **Held Back**: pushed back one stage, rewinding the Hourglass by an Hour.

**The Named (Victory).** From Act II, some districts add a **Named** Elite enemy; each district's entry says whether it is set aside until a card spawns it or shuffled into the encounter deck. Defeating one records its name on your Campaign Log and banks its **Victory X** as Memory, once per campaign per name. The Named return every loop like everything else in Ambergrove; only the first lesson pays.

**The Control token.** The Control token keeps the campaign's books so you do not have to. Use it like this, and do not also apply by hand what it applies for you:
- **Hour.** Each time the Hourglass reaches a new Hour, left-click **Hour** on the Control token. It applies that Hour's changes to Dissonance, to the [static] tokens and to the Appointed's Approach. You resolve the rest of the Hour's text (encounter draws, moving investigators, and so on).
- **Dissonance.** Each time a card or ability raises or lowers Dissonance, click **Dissonance** (left-click +1, right-click −1). It adds and removes the band's [static] tokens in the chaos bag.
- **Memory on cards.** Click an investigator's **Memory** row when Memory is placed on or removed from that investigator's cards (left-click +1, right-click −1).
- **The Appointed.** Its **Hold Back** button is on its card.
- **Knowledge.** When you record a Knowledge entry, tick it on the Campaign Log token. The Control token records it too and turns the locations it changes to their new side.
- **Victory.** When you defeat a Named enemy, tick it as defeated on the Campaign Log token. The Control token banks its Victory Memory the first time only.
- **Contest.** In the finale, the Control token shows a **Contest** counter (left-click +1, right-click −1).
- **Between loops.** Use **Reset Loop**, **Interlude** and **Clear Board** as **Between Loops** and **Loop Setup** direct.

**Acts of the campaign.**
- **Act I — Learning the Rules:** only each district's first objective (act 1a) is available.
- **Act II — The Shape of the Hour:** each district's second objective (act 2a) opens, the Appointed's Whispers join the encounter deck, and the Named enter the night. The guide tells you when Act II begins.
- **The Last Hour:** the finale, once your Campaign Log records **The Way the Night Breaks.**

---

## CAMPAIGN SETUP

1. **Choose investigators.** The campaign is tuned for 3; it scales from 1 to 4.
2. **Build decks.** Each investigator builds a deck under their deckbuilding rules.
3. **Choose a difficulty** and assemble the chaos bag. On the Control token, click your difficulty's button (**Easy**, **Standard**, **Hard** or **Expert**): it fills SCED's chaos bag with the tokens below. Without SCED, build the bag by hand. Then set **Investigators** on the Control token to the number of investigators.

| Difficulty | Chaos tokens |
|---|---|
| Easy | +1, +1, 0, 0, 0, −1, −1, −1, −2, −2, [skull], [skull], [cultist], [tablet], [elderthing], [autofail], [elder] |
| Standard | +1, 0, 0, −1, −1, −1, −2, −2, −3, −4, [skull], [skull], [cultist], [tablet], [elderthing], [autofail], [elder] |
| Hard | 0, 0, 0, −1, −1, −2, −2, −3, −3, −4, −5, [skull], [skull], [cultist], [tablet], [elderthing], [autofail], [elder] |
| Expert | 0, −1, −1, −2, −2, −3, −3, −4, −4, −5, −6, −8, [skull], [skull], [cultist], [tablet], [elderthing], [autofail], [elder] |

4. **Take the Campaign Log** (the log token, or the pages at the back of this guide). Record your investigators and difficulty. Each investigator starts at **0 Years** and **0 banked Memory**.
5. Turn to **Prologue — The First Hour.**

---

## THE AMBERGROVE MAP

A small inland town under a starless, snagged sky. **The Square** is the hub; travelling along a connection costs **1 Hour** (a Skip).

```
                 THE LIGHTHOUSE
                       |
                 THE SUNKEN ROAD
                       |
   THE FAIRGROUND — THE SQUARE — THE DROWNED CHURCH
                       |
                 THE ALMANAC HOUSE
```

- **The Square** connects to the Fairground, the Drowned Church, the Almanac House and the Sunken Road.
- **The Sunken Road** connects the Square to the **Lighthouse**.
- Each district is a cluster of 2–4 locations. They enter play unrevealed: when an investigator enters an unrevealed location, flip it to its revealed side, and SCED places its clues.

---

## PROLOGUE — THE FIRST HOUR

> You came to Ambergrove for your own reasons: a letter, a debt, a promise, a rumour. You will not remember them, soon.
>
> Ambergrove is a town on no one's way to anywhere: a church, a road out to the shore, a lighthouse no ship has needed in years, a printing house that sells almanacs to three counties. Tonight it is full. The almanac has promised an occultation, a star swallowed by the dark of something passing in front of it, once in a hundred years, and visible from here and nowhere else.
>
> The whole town has turned out for the occultation. Lanterns are strung across the Square. A band is tuning up by the Town Hall steps, and children ride their fathers' shoulders, pointing at a sky that is not quite right.
>
> The almanac promised the occultation at midnight. It is eleven, and the stars are already sliding behind something that has no edge. You will remember everything else.

**Setup**
1. Press **Place** on **The First Hour** box. It lays out:
   - the **Hours** as the agenda deck, Hour I on top;
   - the act **The First Hour**;
   - **The Still Hour** scenario reference card;
   - the encounter deck, face down. Shuffle it;
   - one story card, set aside. Do not read it yet.
2. Put **The Square**, **The Long Pier** and **The Almanac Steps** into play in a row (Place does this). The Square is revealed; the other two are unrevealed. Each investigator begins at **The Square**.
3. The Control token starts a new campaign in the Prologue, at **Hour I** with Dissonance **0**. The chaos bag is in the Calm band.
4. Each investigator draws their opening hand. You are ready to begin.

> **Note.** The Prologue teaches the loop. It cannot be lost: however it ends, the night resets, and how well you did decides one small reward.

**Do not read until the end of the scenario.**

```resolution Resolution 1 — You Saw It Coming (an investigator resolved the Occultation at the Almanac Steps)
> You were standing on the right step at the right second. You watched the occultation complete, and you watched something reach through, and not finish reaching. When the night folds, you fold with it on purpose, and you keep more of it.
- Each investigator gains **2 banked Memory**, and the party gains **1** more banked Memory.
- In your Campaign Log, record **You Are Unstuck.** Act I begins.
- No Years are gained from the Prologue.
- Read **What You Saw**, below.
```

```resolution Resolution 2 — It Caught You Unready (the Hourglass reached Hour IX)
> The occultation completes. Something reaches through, and does not finish reaching. The night folds. You are standing in the Square again at eleven o'clock, salt on your tongue, and you are the only ones who feel the seam.
- Each investigator gains **2 banked Memory.**
- In your Campaign Log, record **You Are Unstuck.** Act I begins.
- No Years are gained from the Prologue.
- Read **What You Saw**, below.
```

```resolution No Resolution — The First Death (every investigator was defeated)
> The last thing you see is the sky finishing what it started: the final star sliding behind the edgeless dark. There is no pain. There is a long, patient silence, like a held breath. Then a band is tuning up, and the lanterns are being lit, and it is eleven o'clock. You remember dying. You remember all of it.
- Each investigator gains **2 banked Memory.**
- In your Campaign Log, record **You Are Unstuck.** Act I begins.
- No Years are gained from the Prologue. This first death is free. None of the others will be.
- Read **What You Saw**, below.
```

### What You Saw

> Salt on your tongue. The same lanterns, the same band, the same children on the same shoulders. You look at one another across the Square and see it in each other's faces: you all remember. No one else does. A woman in a green shawl asks if you are feeling well.

**The investigators must decide** (choose one):
- **Tell the town.** Stand on the Town Hall steps and tell them what is coming, though they will not believe a word. Record in your Campaign Log: *The town was warned.*
- **Tell no one.** Whatever this is, it is yours to carry, and a secret kept is a card still in the hand. Record in your Campaign Log: *You kept the night to yourselves.*

Then, on the Control token, click **Reset Loop**. This ends the Prologue; the Prologue does not count as a loop. Turn to **The Loop**, read **Between Loops**, and begin Loop 1.

---

## THE LOOP

Play every loop from here on the same way: set it up, play the night across the districts you choose, and when it ends read the loop's resolution, then **Between Loops**.

**Loop Setup**
1. If a loop or the Prologue just ended, click **Clear Board** on the Control token. Every card the boxes laid out, and every token on those cards, leaves the table. Each investigator heals all damage and horror, shuffles every card they own back into their deck and draws a new opening hand.
2. Press **Place** on **The Square (Town Hall)** box. It lays out the Square's locations, the **Hours** (agenda deck, Hour I on top), the scenario reference card, the shared encounter deck, the Square's act deck (in the district row between the mythos area and the map) and, set aside, the Appointed's cards.
3. Apply every Knowledge entry on your Campaign Log that changes the Hours (for example, an Hour removed from the Occultation is taken out now).
4. The Control token now shows **Hour I** and Dissonance at the **scar**: the number of loops completed so far, maximum **6** (Reset Loop set both).
5. **Choose the districts** you mean to visit this loop. For each one, press **Place** on its box: its locations join the map unrevealed (except any its entry says are revealed), its act deck takes its own spot in the district row, and its encounter set and any Named enemy go face down just below the mythos area. Shuffle each district's encounter set into the encounter deck. Leave each Named enemy set aside unless its district's entry says otherwise.
6. **Act II onward:** shuffle the Appointed's Whispers into the encounter deck.
7. The Appointed begins **Unseen**, set aside. Each investigator begins at **The Square**.
8. The first time you enter a district in this loop, read its entry in **The Districts**.
9. If your Campaign Log records *You refused the ticket*: the first time a **Lost Hour** is drawn this loop, cancel its revelation and discard it. The fair's clock stops for you, once.

> Salt on your tongue. Lanterns across the Square. A band tuning up. It is eleven o'clock, and you have been here before.

**Do not read until the loop ends.**

A loop ends the moment one of its endings happens. Check them **top to bottom** and read the first that happened. Then read the **district resolution** for each district whose box you placed this loop (see **The Districts**), and then **Between Loops**.

```resolution Resolution 2 — The Seam Tears (Dissonance reached 18)
> You leaned on what you remembered until the night could feel the weight. Every clock in Ambergrove stops on the same wrong second. The sky does not fold this time. It splits, and you fall back through the gap to eleven o'clock, bleeding time.
- The loop ended in danger: each investigator's **+1 Year** for Dissonance 12 or higher applies.
- In your Campaign Log, mark this loop **Torn**.
- If this is the **second or later** Torn loop, read this instead of the usual interlude story: *The town has learned your shape. Strangers at the fair stop mid-laugh when you pass, and do not know why.*
- Read the district resolutions, then proceed to **Between Loops.**
```

```resolution No Resolution — The Night Keeps You (every investigator was defeated)
> The dark finishes what it started. For a long moment there is nothing: no Square, no bell, no salt. Then the lanterns are being lit again, and you are standing where you always stand, and your hands will not stop shaking.
- Each investigator gains the **+1 Year** for being defeated.
- In your Campaign Log, mark this loop **Taken**.
- If this is the **second or later** Taken loop, read this instead of the usual interlude story: *You have died in this town more times than you can hold in your head at once. Each of you keeps one of those deaths close, like a stone in a pocket, and it is heavier every time.*
- Read the district resolutions, then proceed to **Between Loops.**
```

```resolution Resolution 1 — The Appointed Hour (the Hourglass reached Hour IX)
> The occultation completes on time, exactly as the almanac promised. Something reaches through the edgeless dark, and, again, does not finish reaching. The night folds along its old crease. Eleven o'clock. Salt. The lanterns.
- In your Campaign Log, mark this loop **Closed at the Hour**.
- If you recorded **at least one** new Knowledge entry this loop, add to the interlude story: *It is the first night that felt like progress. You hold on to that.* If you recorded **none**, add: *Another night spent. The town did not notice you at all, which is its own kind of warning.*
- Read the district resolutions, then proceed to **Between Loops.**
```

---

## BETWEEN LOOPS

Run this after every reset, and once after the Prologue. First, if you have not already, click **Reset Loop** on the Control token; then click **Interlude** on it for steps 1 to 3.

1. **Age.** (Skip this step after the Prologue.) Each investigator adds their Years for the loop just ended (see Campaign Rules). Apply any bracket change now; the first time an investigator enters Weathered, choose their locked drift on the Campaign Log. An investigator who reaches 18 Years ages out.
2. **Bank Memory.** Move all Memory on cards to banked Memory. Reduce banked Memory to **18** if it is higher.
3. **Spend.** Spend banked Memory from the shared pool: buy Recollections at their listed Memory cost, and level up eligible cards at 1 Memory per level, within each investigator's deckbuilding access. Upgraded cards replace their base versions.
4. **Record.** Update the scar (loops completed, maximum 6), the Knowledge Track, the dead and any choices on your Campaign Log.
5. **Check the Act.** If your log now holds **3 or more** Knowledge entries from districts' first objectives, or you have completed **Loop 3**, and Act II has not begun: Act II begins. Read **The Shape of the Hour**.
6. **Read** the interlude story below. On the Control token, click **Begin Next Loop**, then begin the next loop at **Loop Setup**.

> **After the Prologue and the first loop.** You wake at First Dark with the taste of salt and the certainty that you have done this before. The others feel it too. No one in Ambergrove will believe you.

> **The Shape of the Hour** (when Act II begins). The town has begun to notice that you notice. Somewhere past the edge of the Square, something that has been waiting politely decides to stop waiting. From the next loop, each district's second objective opens, the Appointed's Whispers join the encounter deck, and the Named enter the night.

> **The fourth loop and later** (read instead of the first interlude story, whenever no other story applies). The night is the same. You are not. You catch yourself counting the hours in your sleep, and waking at eleven before the bell. The town is exactly as young as it was the first time. Every morning that never comes, you are a little older than it.

**Age stories.** The first time any investigator reaches a new age bracket, read its story aloud as well.
- **Weathered** (5 Years): *In the glass of the Hall of Mirrors there is a line beside your mouth that was not there when you came. The reflection notices it before you do.*
- **Elder** (10 Years): *The Winding Stair has more steps than it used to. It does not. You count them every loop, and every loop it takes you longer to reach the top.*
- **Ancient** (15 Years): *A child in the Square takes your hand to help you down the Town Hall steps. You have known this child for a hundred nights. To them you are a kind old stranger, and always will be.*
- If an investigator **ages out** (18 Years): *They do not wake at First Dark. You find them on the Sunken Road, walking in the same direction as the others, hood up. They do not turn when you call their name.*

> **Before the finale** (once your log records The Way the Night Breaks). You know the shape of the night now. You know what it costs to end it. The only question left is who pays. You may attempt **The Last Hour** during any loop from now on, or keep looping to prepare, knowing each loop costs years.

---

## THE DISTRICTS

Read a district's entry the first time you enter it in a loop. Its objectives are printed on its act deck: act 1a is always available, act 2a only in Act II. When an act tells you to record something in your Campaign Log, do so at once. It lasts through every reset.

**District resolutions.** When a loop ends, after its loop resolution, read one resolution for **each district whose box you placed this loop** (always including The Square). Check each district's resolutions **top to bottom** and read the first that applies. They tell what the night did with what you found there; anything they describe was already recorded on the act's back when you completed it, so they add nothing further to the log unless they say so.

**Choices.** The first time you complete a district's deep objective, its resolution asks the investigators to decide between two courses. Record the choice in your Campaign Log (**Choices**). Every choice echoes somewhere else in Ambergrove, and every one of them is waiting for you in **The Last Hour**.

### The Lighthouse

> The lamp at the end of the Sunken Road is dark. It has never, in any loop, been lit, and the keeper cannot remember why he didn't light it. The stair winds up into the smell of old oil and older salt.

*If your Campaign Log already records The Lamp Was Never Lit, read this instead:*

> The beam you lit last time is gone, as if it had never been. The wick is dry again, the glass is cold, and the stair has forgotten your footsteps. But your hands have not forgotten the way up.

*If your Campaign Log records The walkers keep their ring, add:*

> Far below, on the Sunken Road, the walkers have stopped. Every hood is turned toward the tower, waiting, as though someone once promised them a light.

- When an investigator lights the lamp this loop, exhaust each Echo in the Sunken Road district.

**When you Place this box:** The Lantern Room (revealed), The Winding Stair and The Keeper's Quarters join the map. Shuffle its encounter set (The Dark That Waits, Something on the Stair) into the encounter deck. Its act deck: **Light the Lamp** (1a) and **The Ninth Death** (2a).

**Do not read until the loop ends.**

```resolution Resolution 1 — The Ninth Line (you recorded The Keeper's Ninth Death this loop)
> You carry the logbook down with you, though you know it will be back on its shelf at eleven o'clock. Eight deaths, each dated tonight, each in the same careful hand. The ninth line is ruled and waiting. Whoever keeps this light has been dying for it a long time, and has been writing it down so that someone, someday, would read it.
- If **Elias Warde** is one of your investigators, add: *Elias does not speak on the walk back. At the Square he stops, and says, to no one, "Eight. I thought it was more." Then he goes on.*
- **The investigators must decide** (choose one):
  - **Sign the ninth line.** One investigator writes their own name on the waiting line, so that whoever keeps this light next is someone who chose it. The book takes its due at once: that investigator gains **1 Year** at this interlude. Record in your Campaign Log: *<that investigator> signed the ninth line.*
  - **Leave it blank.** Nobody else will be written into this book. Record in your Campaign Log: *The ninth line was left blank.*
```

```resolution Resolution 2 — A Light on the Water (you recorded The Lamp Was Never Lit this loop)
> For the rest of the night the beam swept the water, and the town below seemed to hold its breath beneath it. Then the night folded, and the light went out with everything else. It does not matter. You have done it once. The lamp will remember being lit, the way a door remembers being opened.
- If **Elias Warde** is one of your investigators, add: *Elias kept his hand on the lamp's brass until the last second. He still cannot remember why he left it dark. He is beginning to suspect that he was never given the chance.*
```

```resolution Resolution 3 — The Keeper's Round (you completed no objective here this loop, but your log records one of the Lighthouse's facts)
> You climbed the stair again, and the stair was the same. There is a kind of comfort in it now: the same smell of oil, the same cold glass, the same dark water. Whatever the Lighthouse still has to tell you, it will keep until you come back for it.
```

```resolution No Resolution — The Dark Keeps Its Lamp (your log records none of the Lighthouse's facts)
> You never reached the light. From the road, looking back, the tower stands against the wrong sky like a finger raised for silence. Somewhere at the top of it, you are sure, something is waiting to be lit.
```

### The Drowned Church

> The bell tolled thirteen times at the first occultation. It should toll twelve. Water stands in the nave to the height of the pews, and the congregation is still kneeling in it.

*If your Campaign Log already records The Thirteenth Toll, read this instead:*

> You count the strokes before they come. Twelve, and the pause, and the thirteenth. The congregation kneels in the black water exactly where it knelt before. None of them look up. The register in the Vestry has closed itself again.

*If your Campaign Log records You have spoken the name, add:*

> The kneeling congregation is murmuring now, all of them, the same two syllables over and over under the black water. You know the word. You were the one who said it first.

**When you Place this box:** The Nave, The Belfry, The Vestry and The Flooded Crypt join the map. The Flooded Crypt stays sealed until a card opens it. Shuffle its encounter set (The Drowned Choir, Thirteen, Rising Water) into the encounter deck. Its act deck: **Why Thirteen?** (1a) and **The Hour Was Wrong** (2a). **Act II:** its Named enemy stays set aside until its act spawns it.

**Do not read until the loop ends.**

```resolution Resolution 1 — The Page Beneath the Water (you recorded The Hour Was Wrong this loop)
> You come up out of the crypt with the page held flat against your chest, water running out of your sleeves. One hour. The whole night hangs on one misprinted hour. When the loop folds, you feel it fold differently, as though a pleat in the night has been pressed out and will not come back.
- If you defeated **The Bell-Ringer Beneath** this loop, add: *Below the flooded steps the rope hangs slack. For the rest of the night, and only for the rest of the night, the bell did not ring at all.*
- If **Seraphine Vale** is one of your investigators, add: *Seraphine stood in the nave with her palm against the cold stone until the water rose. "I heard it," she says afterward. "The first time. I heard it strike thirteen, and I reached for it."*
- **The investigators must decide** (choose one):
  - **Carry the page to the Press.** Where the wrong hour was set, the right one can be set again. Record in your Campaign Log: *The true page reached the Press.*
  - **Read it to the drowned.** They have knelt in the water all night waiting for someone to tell them the hour. Record in your Campaign Log: *The drowned heard the true hour.*
```

```resolution Resolution 2 — Twelve Strokes and One (you recorded The Thirteenth Toll this loop)
> You close the register on the torn thirteenth stroke. Someone in this church knew the bell had rung wrong, and pressed so hard the nib went through. When the bell tolls the next hour, you listen to the thirteenth ring with new ears. It is not an echo. It is a count that does not end.
- If **Seraphine Vale** is one of your investigators, add: *Seraphine does not look at the belfry for the rest of the night. When you ask why, she says only that she has heard that bell before, and not in this town.*
```

```resolution Resolution 3 — Kneeling Water (you completed no objective here this loop, but your log records one of the Drowned Church's facts)
> The church kept its secrets tonight, and kept its congregation, kneeling and patient in the black water. You know more of it than they do. It is less comforting than it should be.
- If you defeated **The Bell-Ringer Beneath** this loop, read Resolution 1's first add-on instead of this resolution's story.
```

```resolution No Resolution — Thirteen (your log records none of the Drowned Church's facts)
> You leave with the bell still counting behind you. Twelve. Thirteen. You do not remember the moment you started counting along with it, under your breath, like a prayer you were taught as a child.
```

### The Sunken Road

> The road between the town and the shore. The same figures walk it every loop, in the same direction, hoods up, never arriving. Some of them turn to look at you now.

*If your Campaign Log already records The Road Remembers, read this instead:*

> The walkers are already on the road when you reach it, as they always are, going the way they always go. You know the milestones by heart now, counting up toward the shore and down toward the town. One of the hooded figures slows, just slightly, as you pass.

*If your Campaign Log records that an investigator signed the ninth line, add:*

> One of the walkers carries a book under their arm, bound in oilcloth like the keeper's log. As you pass, they turn it so you can see the spine. There is fresh ink on it.

**When you Place this box:** The Milestones, The Low Bridge and The Turning join the map. Shuffle its encounter set (The Waiting Congregation, The Bridge Remembers) into the encounter deck. Its act deck: **Walk It Backward** (1a) and **Who Walks Beside You** (2a).

**Do not read until the loop ends.**

```resolution Resolution 1 — Worn Smooth (you recorded Who Walks Beside You this loop)
> You cannot stop seeing the face beneath the hood, worn smooth as a step, and the ring on the hand that rose to stop you. When the night folds you look for them in the crowd at the Square, and do not find them. They are on the road. They are always on the road. You wonder, for the first time, how many loops it takes to wear a face away.
- If **"Birdie" Okonkwo** is one of your investigators, add: *Birdie's compass swung toward the walker and held there, steady as north, until the night ended. She has not put it away since.*
- **The investigators must decide** (choose one):
  - **Take the ring.** The walker opens their hand and lets it go, as if it had been waiting a long time to be carried somewhere else. Record in your Campaign Log: *You carry the walker's ring.*
  - **Leave it with them.** It is the last thing they have that is theirs. Record in your Campaign Log: *The walkers keep their ring.*
```

```resolution Resolution 2 — Against the Walkers (you recorded The Road Remembers this loop)
> You walked the road backward, and the road let you. The walkers did not stop you; some of them, you are almost sure, stepped aside. Whatever this road is, it has learned your shape, and you have learned its. You will not lose an hour on it again.
- If **"Birdie" Okonkwo** is one of your investigators, add: *"I came in on this road," Birdie says. "The wrong night. I remember the milestones counting down." No one asks her what they were counting down to.*
```

```resolution Resolution 3 — Same Direction (you completed no objective here this loop, but your log records one of the Sunken Road's facts)
> The walkers went the way they always go, and so did the night. You watched them from the milestones for a while. None of them arrived. None of them ever will, unless someone changes the road.
```

```resolution No Resolution — Hoods Up (your log records none of the Sunken Road's facts)
> You never learned where the walkers are going. At the end of the night they were still walking, and so, you realize as the night folds, were you: the same direction, hood of darkness up, not quite arriving.
```

### The Square

> The hub of the night. The sheriff stands on the Town Hall steps giving the same speech to the same crowd. He has been dead since ten o'clock. No one has noticed but you.

*If your Campaign Log already records The Sheriff Is Already Dead, read this instead:*

> The sheriff clears his throat on the Town Hall steps and begins the speech you know by heart. You mouth the words along with him. Down in the Well, you know exactly what he is. The crowd applauds on cue.

*If your Campaign Log records The town was warned, add:*

> Near the back of the crowd a woman in a green shawl is not watching the sheriff. She is watching you, frowning, the way you frown at a word on the tip of your tongue.

**Every loop:** the Square's box is placed in Loop Setup, so its locations (The Square and The Town Hall Steps revealed), encounter set (The Same Speech, The Crowd Turns) and act deck are always in play. Its act deck: **The Sheriff Is Already Dead** (1a) and **The Vote That Never Ends** (2a). **Act II:** its Named enemy stays set aside until its act spawns it.

**Do not read until the loop ends.**

```resolution Resolution 1 — Carried, Unanimous (you recorded The Vote That Never Ends this loop)
> Every name in Ambergrove, each in its own hand. You have walked past these people a hundred times: the baker, the band, the children on their fathers' shoulders. They signed. They wanted the door open, and they agreed to a price, and then they forgot. The applause for the sheriff's speech sounds different to you now. It sounds like a vote being taken again.
- If you defeated **What Wears the Sheriff** this loop, add: *It did not fall like a man. It folded, and for a moment the Town Hall steps were empty. Then the speech began again, from a mouth that was only a sheriff's.*
- If **Seraphine Vale** is one of your investigators, add: *Seraphine ran a finger down every name in the ledger. Hers is not there. She is not relieved.*
- If your Campaign Log records *The town was warned*, add: *One name near the bottom has been struck through, fresh, the ink still wet. Somebody listened to you, once, and took their name back.*
- **The investigators must decide** (choose one):
  - **Tear out the page.** No debt the town cannot remember agreeing to should be collected. Record in your Campaign Log: *The vote was torn out.*
  - **Leave the vote standing.** A promise, however foolish, is how this began. It may be how it ends. Record in your Campaign Log: *The vote still stands.*
```

```resolution Resolution 2 — The Body in the Well (you recorded The Sheriff Is Already Dead this loop)
> You looked down the Well and saw him, and then looked up at the steps and saw him again, giving his speech to the lanterns. The town is not just repeating. Something is wearing it, the way a hand wears a glove. You spend the rest of the night studying faces and trying not to wonder which of them are still their own.
```

```resolution Resolution 3 — The Same Speech (you completed no objective here this loop, but your log records one of the Square's facts)
> The speech ended the way it always ends. The crowd cheered the way it always cheers. You stood at the back and knew what they did not, and the knowing sat in you like a stone.
- If you defeated **What Wears the Sheriff** this loop, read Resolution 1's first add-on instead of this resolution's story.
```

```resolution No Resolution — Eleven O'Clock (your log records none of the Square's facts)
> The lanterns, the band, the sheriff on the steps. You passed through the Square as everyone passes through it tonight, and it let you go. At the end of the night you are back in it, as you always will be, until you learn what it is hiding.
```

### The Fairground

> The occultation fair, mid-celebration, forever. The wheel turns. The music never finishes its tune. The ticket-taker smiles at you like he has been expecting you for a very long time.

*If your Campaign Log already records The Wheel Still Turns, read this instead:*

> The music starts the same bar it started last time and gets no further. The wheel is turning. You know how the town looks from the top of it now, and you know how still it is. The ticket-taker lifts his hat to you, as he always does, as if you were old friends.

*If your Campaign Log records You kept the night to yourselves, add:*

> The ticket-taker catches your eye across the crowd and touches one finger to the side of his nose. A man who keeps a secret knows another who does.

**When you Place this box:** The Wheel, The Hall of Mirrors and The Ticket Booth join the map. Shuffle its encounter set (The Wheel's Turn, Your Reflection Lies) into the encounter deck. Its act deck: **The Wheel Still Turns** (1a) and **The Ticket-Taker's Bargain** (2a). **Act II:** shuffle its Named enemy into the encounter deck with its encounter set; it spawns at **The Wheel** when drawn.

**Do not read until the loop ends.**

```resolution Resolution 1 — The Fare (you recorded The Ticket-Taker's Bargain this loop)
> He named no price. He only told you there was one, and that you would know it when you could afford it. As the night folds you can still hear the fair music playing its one unfinished bar, and under it, very softly, the punch of a ticket.
- If you defeated **The One Who Rides Forever** this loop, add: *The wheel stopped for the first time in a hundred years. It started again at eleven o'clock, as though nothing had happened. Something had.*
- If **Cass Lindqvist** is one of your investigators, add: *He called Cass by name and asked after her debts. Cass has never told anyone in Ambergrove about her debts. She spends the rest of the night working out the odds on how he knew, and does not like any of them.*
- **The investigators must decide** (choose one):
  - **Take the ticket.** He holds it out between two fingers. It is warm, and blank, and it is yours. Record in your Campaign Log: *You hold the ticket.*
  - **Refuse it.** Whatever he is selling, you will find another way out. Record in your Campaign Log: *You refused the ticket.*
```

```resolution Resolution 2 — The Whole Town at Once (you recorded The Wheel Still Turns this loop)
> From the top of the wheel you saw Ambergrove laid out like a hand of cards face up on the table: every street full, no one moving, the church, the road, the dark tower at the end of it. Some part of you has kept that view. You find you can call it up with your eyes closed.
- If **Cass Lindqvist** is one of your investigators, add: *Cass counted the town from up there the way she counts a deck. "Somebody's dealing," she says afterward. "I just can't see the hand."*
```

```resolution Resolution 3 — One More Ride (you completed no objective here this loop, but your log records one of the Fairground's facts)
> The fair went on without you, as it always does, and the tune got no further than it ever gets. The ticket-taker watched you leave. He did not seem disappointed. He seems to have all the time in the world.
- If you defeated **The One Who Rides Forever** this loop, read Resolution 1's first add-on instead of this resolution's story.
```

```resolution No Resolution — The Tune That Doesn't End (your log records none of the Fairground's facts)
> Lights, music, the wheel going round. You walked through the fair and it walked through you, and when the night folded you could still hear the tune, stopping, as it always stops, one note short of the end.
```

### The Almanac House

> Where the wrong almanac was printed. Stacks of it still wait by the press, bound and dated, the hour of the occultation set in fresh black type. At the back, a study door that no key in Ambergrove fits.

*If your Campaign Log already records What the Almanac Hid, read this instead:*

> The press has set the wrong hour again overnight, as if you had never touched it. The stacks are bound and waiting. But the study door at the back is not as closed as it was. You can feel it, the way you feel someone behind you in an empty room.

*If your Campaign Log records The true page reached the Press, add:*

> The page you brought up out of the flooded crypt lies on the bed of the press, dry now, and the type beside it has been set in the right order. Someone has been working here in the loops you were not watching.

- Once per loop, travel between The Square and the Almanac House costs 0 Hours (you know the hour here now).

**When you Place this box:** The Reading Room, The Press and The Sealed Study join the map. The Sealed Study stays sealed until a card opens it. Shuffle its encounter set (The Page That Wasn't, Ink Runs Backward, The Study Door) into the encounter deck. Its act deck: **What the Almanac Hid** (1a) and **The Appointed's Name** (2a).

**Do not read until the loop ends.**

```resolution Resolution 1 — Written Once (you recorded The Appointed's Name this loop)
> You did not say it aloud. None of you did. But you carried it out of the Study all the same, and the night felt you carry it. For the rest of the loop the thing on the hill came on more slowly, like a guest who has heard their name called from inside the house and is no longer sure of their welcome.
- If **Dr. Ayako Sōma** is one of your investigators, add: *Ayako knows the language it is written in. She has spent her life trying to find someone who spoke it. She has found them.*
- If **Seraphine Vale** is one of your investigators, add: *Seraphine knows the name. She heard it once before, on the night she reached for the sky and something, very politely, reached back.*
- **The investigators must decide** (choose one):
  - **Speak the name.** Once, aloud, into the empty study, so that it knows you know. The night takes its price for that at once: each investigator gains **1 Year** at this interlude. Record in your Campaign Log: *You have spoken the name.*
  - **Keep it unspoken.** A name held back is a name you can still use. Record in your Campaign Log: *The name is kept unspoken.*
```

```resolution Resolution 2 — The True Hour (you recorded What the Almanac Hid this loop)
> For the rest of the night the reprinted pages lay warm on the press, printed with the right hour, and the sky above the Almanac House was almost ordinary. Almost. When the night folds, the press will set the wrong hour again. But you know now that the hour was wrong, and that somebody chose it.
- If **Dr. Ayako Sōma** is one of your investigators, add: *In the margin of the reprint, in a hand she recognizes as her own, is a translation note Ayako does not remember writing. It is one word. The word is "wait."*
```

```resolution Resolution 3 — Fresh Black Type (you completed no objective here this loop, but your log records one of the Almanac House's facts)
> The stacks were bound and dated. The press was set. You read what you could and left the rest, and at the back of the house the study door stayed exactly as closed as it chose to be.
```

```resolution No Resolution — Bound and Dated (your log records none of the Almanac House's facts)
> You leave the Almanac House with ink on your fingers and nothing in your hands. The stacks go on waiting by the press, bound and dated for tonight, the hour of the occultation set in fresh black type. Someone printed that hour. Someone wanted it believed.
```

---


## FINALE — THE LAST HOUR

**Only once your Campaign Log records The Way the Night Breaks.** During any loop from then on, an investigator at **The Sealed Study** may declare **The Last Hour**. If the Hourglass reaches Hour IX while your log records The Way the Night Breaks, you may begin the finale then instead of resetting.

> You have lived this night more times than you can hold. You know the toll, the road, the wrong sky, the name. You know what the town agreed to: an appointment made in ink and never kept.
>
> You feel the loops in your body now. In the joints of your hands, in the grey at your temples, in the way the stair at the lighthouse has grown longer every time you climbed it. The town has not aged a single night. You have aged all of them.
>
> Out on the hill, the guest has stopped being polite. All that is left is to keep the appointment, and to decide what you are willing to spend.

**Check your Campaign Log** and apply each entry that is recorded:
- *You have spoken the name:* gain **2** contest progress now. It already knows that you know.
- *The drowned heard the true hour:* after setup, rewind the Hourglass by **1** Hour (no Hour resolves). Below the church, a congregation is keeping time for you.

**Setup**
1. Press **Place** on **The Last Hour** box. Its act, **Contest the Crossing**, takes the act spot; its set-aside stack goes to the mythos area's lower corner. Do not read the set-aside cards.
2. Resolve **Contest the Crossing**'s opening text: the Appointed's Approach advances to Arrived, and it manifests at the location farthest from the investigators if it is not already on the board. **Dissonance does not reset.** If the finale began at Hour IX, set the Hourglass back to Hour V; no Hour resolves.
3. The objective is to **contest the crossing**: gain contest progress equal to **4 × investigators (12 at three players)** before the Hourglass reaches Hour IX. Each **Hold Back** success, each deep Knowledge entry spent and each investigator's first arrival at The Sealed Study during the finale adds 1.
4. Keep these in mind during the finale, if your log records them:
   - *You carry the walker's ring:* once during the finale, an investigator may take the **Hold Back** action and succeed without a test.
   - *The name is kept unspoken:* once during the finale, when the Hourglass would advance, an investigator may speak the name instead: the Hourglass does not advance.

> **Halfway.** The first time your contest progress reaches half its target, read aloud: *Every clock in Ambergrove stops, then starts again, ticking backward. For one breath you are all the ages you have ever been at once: young and tired and old, standing on every step of every stair you ever climbed in this town. Then the moment passes, and the thing on the hill is closer, and so is the dawn.*

**Do not read until the end of the finale.**

When the finale ends, find where you stand:
- **If the contest was reached**, the night can be ended. **The investigators must decide how.** Choose **one** of Resolutions 1, 1b, 2, 3 or 4 whose condition you meet and read it. Resolution 4 is always available.
- **If the contest was not reached**, read Resolution 5 if banked Memory is 12 or more, otherwise Resolution 6.

Then read **Epilogue — What the Years Took.**

```resolution Resolution 1 — Take Its Place (contest reached, and an Ancient investigator, or the investigator who signed the ninth line, is present)
> Someone has to hold the night's crease closed from the inside. The one who has spent the most of themselves here knows it before anyone speaks. They take the lamp, or the pen, or simply each of your hands in turn, and they walk out onto the hill to meet the guest, and they do not come back down.
- Choose one Ancient investigator, or the investigator who signed the ninth line, to become the night's new anchor: kept behind, decades older, so that the loop ends for everyone else. Mark them **kept as anchor** on your Campaign Log.
- If your log records **Who Walks Beside You**, instead: the ones who walk the Sunken Road come up the hill and take the anchor's weight with you. At dawn the anchor walks out with the others, free and not kept, but **+3 Years** older. If your log also records *The walkers keep their ring*, the walkers carry the anchor almost the whole way: **+1 Year** instead of +3.
- The town wakes for the first time in a hundred years. The campaign is over.
```

```resolution Resolution 1b — Let It In, On Your Terms (contest reached, your log records The Ticket-Taker's Bargain but not "You refused the ticket," and banked Memory is 12 or more; 10 or more if you kept the night to yourselves)
> He punches your ticket without looking. He has been expecting you for a very long time. The door opens, not the way the town opened it, begging, but the way you open a door to a guest you have decided to receive. It comes in. It sits down at the edge of the Square in the lantern light, and it is smaller than you feared, and it is very, very old.
- The door opens on your terms, not the town's, and the night finally ends.
- The fare is everything you carried: banked Memory drops to **0**, and each investigator present ages **+2 Years**. If your log records *You hold the ticket*, the fare is already half paid: **+1 Year** instead.
- Ambergrove wakes to a morning with one more guest in it. The campaign is over.
```

```resolution Resolution 2 — Close the Door (contest reached, your log records The Vote That Never Ends and The Appointed's Name, and the vote still stands)
> You pay the price the town agreed to and never paid. It is paid in years, as every price here is. You feel them go, all at once, like stepping off a stair that is not there. When you look up the sky has an edge again, and the edge is the dawn.
- Everyone present ages **+3 Years** at once, and the door closes. If your log records *The town was warned*, Ambergrove remembers just enough to share the cost: **+2 Years** instead.
- The cleanest true ending, bought with years. The campaign is over.
```

```resolution Resolution 3 — Break Through (contest reached, and your log records The Keeper's Ninth Death)
> You force the night open, and every survivor runs through before it can close. Behind you the Square, the band, the lanterns, the sheriff on his steps, all fold away like a stage set in the rain.
- All surviving investigators escape. The campaign is over.
- But the loop is only paused. Somewhere, some night, an almanac will be printed with the wrong hour again. Was it really ended?
- If your log records *The ninth line was left blank*, read this instead of the last point: *No one is written into the keeper's book. No one is owed. The loop has nothing left to hold, and it does not start again.*
```

```resolution Resolution 4 — Seal by Force (contest reached)
> The door is sealed, but not closed. Someone has to lean against it until the dawn comes, and the dawn is a long way off.
- Choose one investigator. They age out holding the door shut. The campaign is over.
- If your log records *The vote was torn out*, no debt is owed behind that door, and it pushes back with nothing but its own weight. The chosen investigator holds it until morning and walks away **+3 Years** older instead of aging out.
```

```resolution Resolution 5 — Next Time (contest not reached, banked Memory 12 or more)
> The loop wins tonight. The guest keeps its appointment, and the night folds along its old crease. But you have carried so much of it now that the fold does not quite close over you. You wake at First Dark closer than ever, older than ever, and certain.
- Proceed to **Between Loops**, then begin a new loop. You may attempt the finale again.
- Do not read the epilogue yet.
```

```resolution Resolution 6 — The Loop Wins (contest not reached, banked Memory below 12)
> The night takes you. The town keeps repeating, and somewhere on the Sunken Road, walking the same direction and never arriving, there are new faces beneath the hoods.
- The campaign ends.
```

**Epilogue — What the Years Took.** Unless you read Resolution 5, read one line for each investigator who survived, matching the age bracket on your Campaign Log.
- **Prime:** *You leave Ambergrove as young as you came, and no one on the train believes how tired you are.*
- **Weathered:** *There is grey in your hair that was not there the night you arrived. You keep it. It is the only proof.*
- **Elder:** *Your hands shake a little now in the mornings. You spent years on one night, and you would spend them again.*
- **Ancient:** *The station master helps you down from the carriage and asks if you have family meeting you. You had family, once, a hundred years ago tonight.*
- An investigator **kept as anchor** has no epilogue. On clear nights in Ambergrove, the lamp at the end of the Sunken Road is lit.

---

## DIFFICULTY AND PLAYER COUNT

- **3 investigators:** every value as printed (Dissonance bands at 6 / 12, reset at 18, Memory cap 18, contest 12, scar cap 6).
- **2 or 4 investigators:** the reset comes at **6 × investigators**; the Glitch band starts at one-third of the reset and the Noticed band at two-thirds (**4 × investigators**). The Memory cap is **6 × investigators**, the contest is **4 × investigators**, and the scar cap is one-third of the reset.
- **1 investigator:** the reset comes at **9** and the Noticed band at **6**; Glitch starts at **3**. The Memory cap is **6**, the contest **4**, the scar cap **3**.
- The Control token applies all of these for you.
- The campaign is tuned for about 6–8 loops.
