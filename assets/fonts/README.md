...
| **Crimson Pro** (`CrimsonPro-Regular/Italic/Bold/BoldItalic.ttf`) | **The default body font** — rules text, traits, flavor, keywords. An Aldine oldstyle from the same Garamond lineage as Arno Pro, so card text reads like the printed cards. Static instances cut from Google Fonts' variable font (`fontTools.varLib.instancer`). | OFL 1.1 (`CrimsonPro-OFL.txt`) | **yes** |
| **Nimbus Roman No9 L** (`NimbusRomNo9L-Reg/RegIta/Med/MedIta.otf`) | Committed body fallback (a Times clone), and the face for the resolution arrow (→) that Crimson Pro lacks. | URW / freely redistributable (GhostScript AFPL, part of the standard free font set) | **yes** (vendored from the element kit) |
...
3. the baked-in default stack — titles + cost: Arkhamic → Teutonic; stat
   numerals: Bolton Bold → Bolton → bold body; body: Arno Pro if the owner drops licensed copies in, else **Crimson Pro**
   (all four styles, committed — the default for every card), then Nimbus;
