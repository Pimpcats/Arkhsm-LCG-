...
Dry runs mark "dry:id:seed" instead: a dry rehearsal resumes within dry mode
but never blocks the real GPU run that follows it.
"""
import json
import os
import tempfile
...
class Ledger:
    ...
    def is_done(self, job_id, seed, dry=False):
        k = self.key(job_id, seed)
        return k in self.done or (dry and "dry:" + k in self.done)

    def mark(self, job_id, seed, dry=False):
        self.done.add(("dry:" if dry else "") + self.key(job_id, seed))
